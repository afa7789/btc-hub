...
export interface HalvingConfig {
  divId: string;
  halvingTimestamps: number[];
  nextHalving: Date;
  width: number;
  height: number;
  btcData: { timestamp: Date[]; price: number[] } | null;
}

/**
 * Draw Bitcoin Halving Chart with vertical lines and shaded regions
 */
export function halvingDraw(config: HalvingConfig): void {
  const { divId, halvingTimestamps, nextHalving, width, height, btcData } =
    config;
      ...
      svg
        ...
        .call(
          d3
            .axisLeft(btcY)
            .ticks(5)
            .tickFormat((d: number) => `$${d3.format(".2s")(d)}`),
        );
...
/**
 * Draw combined chart with multiple data series
 */
...
/**
 * Draw Dollar Purchasing Power Chart
 */
export function drawDollarPurchasingPowerChart(purchasingPowerData: {
  timestamp: string[];
  value: number[];
}): void {
  if (
...
/**
 * Draw relative growth chart with improved logarithmic scaling
 */
export function drawRelativeGrowthChart(
  dataSets: DataSetConfig[],
  divId: string,
  title?: string,
): void {
  const chartTitle = title || "CRYPTO RELATIVE GROWTH IN USD";
  const chartDiv = document.getElementById(divId);
  if (!chartDiv) {
    console.log(`Chart container with id ${divId} not found.`);
    return;
  ...
  if (useLogScale) {
    ...
    const tickVals = steps.filter((v) => v >= logMin && v <= logMax);
    yAxis.call(
      d3
        .axisLeft(y)
        .tickValues(tickVals)
