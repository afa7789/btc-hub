...
import { formatBillions } from "../../utils/formatters.ts";
...
async function loadData() {
  ...
  try {
    const jsonResponse = await fetch("/datasets/all-the-money/data.json");
    if (!jsonResponse.ok)
...
async function fetchFreshDataInBackground() {
  try {
    ...
    const response = await fetch(
      `/datasets/all-the-money/data.json?t=${Date.now()}`,
    ); // Cache bust
    if (!response.ok) {
    ...
    if (wrapped) {
      ...
      for (let i = 0; i < totalBlocks; i++) {
        ...
        if (isPartialBlock) {
          ...
          // Debug logging
          if (
            item.name.includes("Jeff Bezos") ||
            item.name.includes("Elon Musk")
          ) {
            console.log(
              `📐 ${item.name}: Ratio=${partialBlockRatio.toFixed(3)}, AreaScale=${areaScale.toFixed(3)}, FinalArea=${(areaScale * areaScale).toFixed(3)} (should equal ratio)`,
            );
          }
        }
...
function formatLargeNumber(billions) {
  return formatBillions(billions);
}
...
function initialize() {
  ...
  loadData().then((loadSuccess) => {
    if (wealthData) {
      ...
      if (!loadSuccess) {
        console.warn(
          "Using partially loaded or fallback data - some features may be limited",
        );
      }
    } else {
      console.error("Failed to load any data.");
      document.getElementById("visualization").innerHTML =
        "<p>Failed to load data. Please reload the page.</p>";
    }
  });
}
